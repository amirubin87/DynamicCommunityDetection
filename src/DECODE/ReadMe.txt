...
Parameters:
	...
	Additional:
		betas (1.1,1.2,2.0,3.0) - a list of comma delimited double values, which should be larger than 1.0.
		...
		firstPartMode (0) - only in use if WOCC is used. An integer, one of {0,3,4}. Indicating how to build the first partitioning of the network to communities.
		...
		runMultyThreaded(true) - will run multy threaded when set to true.
		numOfThreads(8) - amount of threads to use when running multy threaded.
		dynamicChoose(true) - when set to false, the objective function is not choosen using the Random Forst classifier, but acording to the "useModularity" flag.
		useModularity(false) - when "dynamicChoose" is set to false this parameter controls if the extended modularity will be used (useModularity = true) or WOCC will be used (useModularity = false).
		debug(false) - additional debug logs will be outputted.
		
Usage examples:
	java -jar DECODE_DYNAMIC ./net.txt ./
			These  are examples with the minimum parameters needed.
	java -jar DECODE_DYNAMIC ./net.txt ./ 1.1,1.2,1.3,1.4,1.5,2.0,2.5,3.0,3.5,4.0 0.8 3 30
		DECODE runs with 10 different beta values. After the third iteration communities which share 70% of their nodes will be merged.
		The algorithm will stop after 30 iterations.
	java -jar DECODE_DYNAMIC ./net.txt ./ 1.5,3.5,5.0,10.0 0.7 4 30 3 99
		DECODE runs with 4 different beta values. The first partitioning is all 3-sized cliques. After the forth iteration communities which share 70% of their nodes will be merged.
