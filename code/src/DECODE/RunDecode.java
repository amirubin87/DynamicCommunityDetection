...
public class RunDecode {
	...
	public static void main(String[] args) throws Exception {		
		...
		if (args.length <2){
			System.out.println("Input parameteres for DECODE: "
					+ "pathToGraph  "
					+ "outputPath  "
			...
			if(!useModularity){
				...
				DecodeW DECODEW = new DecodeW(pathToGraph,betas,alpha,outputPath, iteratioNumToStartMerge, maxIterationsToRun,percentageOfStableNodes,firstPartMode, debug);				
				DECODEW.FindCommunities(runMultyThreaded, numOfThreads);
			}
			else{
				System.out.println("                         Using Modularity.");
				DecodeQ DECODEQ= new DecodeQ(pathToGraph,betas,alpha,outputPath, iteratioNumToStartMerge, maxIterationsToRun, percentageOfStableNodes, debug);
				DECODEQ.FindCommunities(runMultyThreaded, numOfThreads);			
			}
		}
